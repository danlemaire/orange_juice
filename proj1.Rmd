...
variance_of_model_df %>%  
  ggplot(aes(performance)) +
    ...
    geom_errorbarh(aes(x = median(variance_of_model_df$performance), 
                       xmin = median(variance_of_model_df$performance) - sd(variance_of_model_df$performance), 
                       xmax = median(variance_of_model_df$performance) + sd(variance_of_model_df$performance), 
                       y = 10,
                       color = "red"
                       )) +
    ...
    geom_text(aes(x, y, label = x),
              data_frame(x = c(round(median(variance_of_model_df$performance) - sd(variance_of_model_df$performance), 4), 
                               round(median(variance_of_model_df$performance), 4), 
                               round(median(variance_of_model_df$performance) + sd(variance_of_model_df$performance), 4)),
                         y = 11),
              size = c(4,10,4),
              color = "red"
              ) +
...
sens_analysis_df <- train %>% 
  ...
  mutate(PriceCH2 = rep(c(seq(1.5, 2.5, length = 25), rep(NA, 25)), 5),
         PriceCH = ifelse(is.na(PriceCH2), PriceCH, PriceCH2),
         PriceMM2 = rep(c(rep(NA, 25), seq(1.5, 2.5, length = 25)), 5),
         PriceMM = ifelse(is.na(PriceMM2), PriceMM, PriceMM2)
         ) %>% 
  select(-Purchase)

summary(sens_analysis_df$isMean)
...
h2o.predict(h2o_glm, as.h2o(sens_analysis_df)) %>% 
  ...
  ggplot() +
    geom_line(aes(PriceCH2, CH, color = predict)) +
    #geom_smooth(aes(PriceCH2, CH, color = predict)) +
    geom_line(aes(PriceMM2, MM, color = predict)) +
    #geom_smooth(aes(PriceMM2, MM, color = predict)) +
    facet_grid(StoreID~.) +
