...
packages <- list("magrittr", "tidyverse", "caret", "broom", "h2o", "scales", "corrplot", "GGally", "stringr", "Boruta")
...
rf <- h2o.randomForest(train_x, train_y, h2o_train, model_id = "rf_default", 
                       nfolds = 10, fold_assignment = "Modulo", keep_cross_validation_predictions = T) 
gbm <- h2o.gbm(train_x, train_y, h2o_train, model_id = "gbm_default", 
                nfolds = 10, fold_assignment = "Modulo", keep_cross_validation_predictions = T) 
dl <- h2o.deeplearning(train_x, train_y, h2o_train, model_id = "dl_default", 
                        nfolds = 10, fold_assignment = "Modulo", keep_cross_validation_predictions = T) 

stacked <- h2o.stackedEnsemble(train_x, train_y, h2o_train, 
                    base_models = list("default_glm", "gbm_default", "dl_default"), 
...
gbm_tuned <- h2o.gbm(train_x, train_y, h2o_train, model_id = "gbm_tuned", 
        nfolds = 10, fold_assignment = "Modulo", keep_cross_validation_predictions = T, 
...
for (i in 1:30) {
  boot <- h2o_test %>% as_tibble %>% sample_frac(1, replace = T)
  default_glm_results[i] <- ((boot %>% as.h2o %>% h2o.predict(stacked, .))$MinuteMaid %>% as_tibble %>% 
  prediction(., labels = boot$Purchase) %>% performance("auc"))@y.values[[1]][1]
}
...
for (i in 1:30) {
  boot <- h2o_test %>% as_tibble %>% sample_frac(1, replace = T)
  dl_results[i] <- ((boot %>% as.h2o %>% h2o.predict(stacked, .))$MinuteMaid %>% as_tibble %>% 
  prediction(., labels = boot$Purchase) %>% performance("auc"))@y.values[[1]][1]
}
...
for (i in 1:30) {
  boot <- h2o_test %>% as_tibble %>% sample_frac(1, replace = T)
  rf_results[i] <- ((boot %>% as.h2o %>% h2o.predict(stacked, .))$MinuteMaid %>% as_tibble %>% 
  prediction(., labels = boot$Purchase) %>% performance("auc"))@y.values[[1]][1]
}
...
for (i in 1:30) {
  boot <- h2o_test %>% as_tibble %>% sample_frac(1, replace = T)
  gbm_results[i] <- ((boot %>% as.h2o %>% h2o.predict(stacked, .))$MinuteMaid %>% as_tibble %>% 
  prediction(., labels = boot$Purchase) %>% performance("auc"))@y.values[[1]][1]
}
...
#Plot variable importances using standardized coefficients from glm
default_glm@model$coefficients_table %>% 
  as_tibble %>% 
  ...
  ggplot(aes(x = reorder(names, abs(standardized_coefficients)), 
    ...
    coord_flip() +
    labs(title = "Brand loyalty and discounts matter most, and these change over time",
...
train %>% 
  select(Purchase, PriceMM, PriceCH, WeekofPurchase, StoreID) %>% 
  mutate(PriceDiff = PriceMM - PriceCH,
         is_mm = Purchase == "MinuteMaid") %>% 
  group_by(PriceMM, PriceCH, StoreID) %>% 
  summarise(Purchase = mean(is_mm),
            sales = n()) %>% 
  ggplot(aes(PriceMM, PriceCH, color = Purchase, size = sales)) + 
    geom_point() +
    facet_wrap(~StoreID)

train %>% 
  select(WeekofPurchase, PriceCH, PriceMM, StoreID) %>% 
  gather(brand, price, -WeekofPurchase, -StoreID) %>% 
  group_by(WeekofPurchase, brand, StoreID) %>% 
